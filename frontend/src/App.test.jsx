...
test("shows the score after a successful click", async () => {
  fetchScore.mockResolvedValueOnce({ ...BASE_RESULT, overall: 75.5 });
  render(<App />);
...
test("shows an error with a retry button after a transient failure, and retry re-fetches", async () => {
  const err = new Error("Temporary problem reaching the map-routing service. Please try again.");
  err.type = "transient";
  fetchScore.mockRejectedValueOnce(err).mockResolvedValueOnce({ ...BASE_RESULT, overall: 40 });
...
test("a slower superseded request never overwrites a faster later one", async () => {
  ...
  fetchScore.mockImplementationOnce(() => firstPromise).mockImplementationOnce(() => secondPromise);
